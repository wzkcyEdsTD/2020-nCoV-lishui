      ...
      <div class="selectFrame no_select">
        <div v-for="(item, index) in this.tree" :key="index">
          ...
          <span
            ...
            v-if="
              !(
                (tabIndex == 3 && item.fglabel == -1) ||
                (tabIndex == 2 && item.sflabel == -1) ||
                (tabIndex == 1 && item.dqlabel == -1) ||
                (tabIndex == 0 && item.label == -1)
              )
            "
          >
            {{
          ...
          >
            <li
              ...
              v-show="
                !(
                  (tabIndex == 1 && oitem.ytname == -1) ||
                  (tabIndex != 1 && oitem.name == -1)
                )
              "
            >
              <input
              ...
              <p @click="ShowResult(oitem, item), changeTree(oitem)">
                {{ tabIndex == 1 ? oitem.ytname || oitem.name : oitem.name }}
              </p>
              <ToggleSwitch
              ...
              <span
                id="xq"
                v-if="
                  item.label == '疫情分布' || oitem.id == 'glmd' || oitem.id == 'gjmj'
                "
                @click="ShowListxq(oitem, item)"
                >详情</span
              >
              <span id="ssry" v-if="oitem.id == 'xq'" @click="ShowListssry(oitem, item)"
                >实时人员</span
              >
              <span id="fgxq" v-if="oitem.id == 'zjgd'" @click="ShowListfgxq(oitem, item)"
                >返工详情</span
              >
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    changeTree(item, event) {
      this.intercept();
      this.$parent && this.$parent.Listcontact && this.$parent.Listcontact(item);
    },
    stop(e) {
      e.stopPropagation();
    },
