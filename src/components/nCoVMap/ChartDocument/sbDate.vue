      ...
      <ul>
        <li v-for="(item, index) in forceData" :key="index" @click="goLocation(item)">
          <span
            ...
            >{{ ++index }}. 患者：{{
              item.attributes.Name
                ? `${item.attributes.Name.trim().substr(
                    0,
                    1
                  )}*${item.attributes.Name.trim().substr(-1, 1)}`
                : "**"
            }}, {{ item.attributes.Sex }}, {{ item.attributes.Address }},
            {{ item.attributes.Age }}</span
          >
          <span v-if="~['mj', 'mj2', 'gjmj'].indexOf(item.id)"
            >{{ ++index }}.
            {{
              item.attributes.NAME
                ? `${item.attributes.NAME.trim().substr(
                ...
                : "**"
            }}, {{ item.attributes.Sex }}, {{ item.attributes.Address_Department }}</span
          >
          <span v-if="~['gld', 'hmgld', 'hqgld', 'kfgld'].indexOf(item.id)"
            >{{ ++index }}. {{ item.attributes.Name }},
            {{ item.attributes.Address }}</span
          >
          <span
            v-if="
              ~[
            ...
            {{
              item.attributes.Name
                ...
                : "**"
            }}, {{ item.attributes.Address || item.attributes.Sex }}</span
          >
          <span v-if="item.label == '医疗资源'"
          ...
          <span
            ...
            >{{ ++index }}. {{ item.attributes.Name }}</span
          >
          <span v-if="item.label == '人员密集场所' && item.id == 'people_type_3'"
            >{{ ++index }}. {{ item.attributes.name }}</span
          >
          <span v-if="item.label == '人员密集场所' && item.id == 'people_type_7'"
            >{{ ++index }}. {{ item.attributes.Address }}</span
          >
          <span
            v-if="
              item.label == '人员密集场所' &&
              item.id != 'people_type_3' &&
              item.id != 'people_type_7'
            "
            >{{ ++index }}. {{ item.attributes.Name }}</span
          >
          <span v-if="item.id == 'people_type_8' || item.id == 'people_type_9'"
            >{{ ++index }}. {{ item.attributes.Name }}</span
          >
          <span v-if="item.id == 'xq'">{{ ++index }}. {{ item.attributes.name }}</span>
          <span v-if="item.id == 'xqjck'"
            >{{ ++index }}. {{ item.attributes.社区名称 }},
            {{ item.attributes.小区名称 }}</span
          >
          <span v-if="item.id == 'wg'">{{ ++index }}. {{ item.attributes.Name }}</span>
          <span v-if="item.id == 'chanyePlate'"
            >{{ ++index }}. {{ item.attributes.名称 }}（{{
              item.attributes.num
            }}例）</span
          >
          <span
            ...
            {{
              ...
              item.attributes.项目名称 ||
              item.attributes.企业名称 ||
              item.attributes.NAME
            }}</span
          >
        </li>
...
import { xmMenu } from "../config/enums";
